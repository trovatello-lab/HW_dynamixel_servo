...
from .dynamixel_x_servo import DynamixelServos, control_lut, OperatingModes
    ...
    def setup(self):
        ...
        for sname, sid in self.devices.items():
            ...
            self.settings.New(
                sname + "_oper_mode",
                dtype=str,
                choices=[x.name for x in OperatingModes],
            )
    ...
    def connect(self):
        ...
        for sname, sid in self.devices.items():
            ...
            lq.connect_to_hardware(
                ...
                write_func=lambda mode, sid=sid: self.dev.write_operating_mode(
                    sid, mode
                ),
            )
